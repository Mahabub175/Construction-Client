...
const AllGallery = () => {
  ...
  const activeGalleries = galleryData?.results?.filter((item) => item?.status);

  const openModal = (index) => {
    setCurrentImageIndex(index);
    setActiveImageName(activeGalleries[index]?.name || "Untitled");
    setIsModalOpen(true);
  };
        ...
        <div className="columns-2 md:columns-3">
          {activeGalleries?.map((galleryItem, index) => (
            ...
            >
              ...
              />
              {galleryItem.name && (
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                  {galleryItem.name}
                </div>
              )}
