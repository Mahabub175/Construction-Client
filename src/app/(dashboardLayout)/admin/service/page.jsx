...
import ServiceCreate from "@/components/Dashboard/Admin/Service/ServiceCreate";
import ServiceEdit from "@/components/Dashboard/Admin/Service/ServiceEdit";
...
const Service = () => {
  ...
  const { data: serviceData } = useGetSingleServiceQuery(itemId, {
    skip: !itemId,
  });
  ...
  const [deleteBulkService] = useDeleteBulkServiceMutation();
  ...
  const tableData = services?.results?.map((item) => ({
    key: item._id,
  ...
  return (
    <div className="px-5">
      ...
      <Pagination
        ...
        total={services?.meta?.totalCount}
        current={currentPage}
      ...
      <ServiceCreate open={open} setOpen={setOpen} />
      <ServiceEdit itemId={itemId} open={openEdit} setOpen={setOpenEdit} />
      <DetailsModal
        itemId={itemId}
